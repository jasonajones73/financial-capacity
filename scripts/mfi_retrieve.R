## Purpose: Retrieve Median Family Income data from census API
## Author: Jason Jones
...
# Retrive with geometry ----
tracts_sf <- get_acs(geography = "tract", table = cen_var, year = 2018,
                     state = 37, key = api_key, county = 81, geometry = TRUE,
                     survey = "acs5") %>%
  left_join(rename(vars, variable = name)) %>%
  mutate(concept = str_extract(concept, pattern = "\\(([^)]+)\\)$")) %>%
  mutate(concept = str_remove_all(concept, "\\(|\\)")) %>%
...
write_rds(tracts_sf, "data/median_family_income/mfi_sf.rds")
write_rds(county_deatiled_clean, "data/median_family_income/mfi_county.rds")
write_rds(places_detailed_clean, "data/median_family_income/mfi_places.rds")

write_csv(county_deatiled_clean, "data/median_family_income/mfi_county.csv")
write_csv(places_detailed_clean, "data/median_family_income/mfi_places.csv")
write_csv(tracts_sf, "data/median_family_income/mfi_tracts.csv")
